...
func SellerApi(router *gin.RouterGroup) {
	sellerGroup := router.Group("seller/seller/")
	{
		...
		sellerGroup.GET("members/s/:ask_id", seller.AskDetail)      // 查询会员商品咨询详请  members/{ask_id}
		sellerGroup.GET("members/d/:ask_id/reply", seller.AskReply) // 商家回复会员商品咨询 members/{ask_id}/reply
