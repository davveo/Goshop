...
func (redis *RedisClient) Put(key string, value interface{}, expiration time.Duration) bool {
	err := redis.Set(key, value, expiration).Err()
	if err != nil {
		log.Println("redis.Put failed", err.Error())
		return false
	}
	return true
}

func (redis *RedisClient) Remove(keys ...string) bool {
	err := redis.Del(keys...).Err()
	if err != nil {
		log.Println("redis.Remove failed", err.Error())
		return false
	}
	return true
}
...
func (redis *RedisClient) Gain(key string) (bool, string) {
	val, err := redis.Get(key).Result()
	if err != nil {
		log.Println("redis.Get failed", err.Error())
		return false, ""
	} else {
		return true, val
	}
